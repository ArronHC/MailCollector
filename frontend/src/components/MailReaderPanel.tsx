...
function SnoozeOptions({ close, onAction }: { close: () => void; onAction: (date: string) => void }) {
  return <><div className="menu-heading">延后到</div><MenuButton icon={<Clock3 />} label="今天晚些时候" detail="18:00（若已过则明早）" onClick={() => { onAction(snoozeIso("later_today")); close(); }} /><MenuButton icon={<Clock3 />} label="明天" detail="08:00" onClick={() => { onAction(snoozeIso("tomorrow")); close(); }} /><MenuButton icon={<Clock3 />} label="下周" detail="周一 08:00" onClick={() => { onAction(snoozeIso("next_monday")); close(); }} /></>;
}
