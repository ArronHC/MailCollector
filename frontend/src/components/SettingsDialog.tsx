import { Keyboard, LayoutPanelLeft, Mail, RotateCcw, ShieldCheck, SlidersHorizontal } from "lucide-react";
import { Modal } from "./Ui";
import { resetAppSettings, updateAppSettings, useAppSettings } from "../settings";
  ...
  return <Modal open={open} title="设置" onClose={onClose} className="settings-modal">
    <div className="settings-shell">
      ...
      <section className="settings-section">
        ...
        <SettingRow title="阅读字号" detail="仅影响邮件正文阅读区域。"><select value={settings.readingFontSize} onChange={(event) => updateAppSettings({ readingFontSize: event.target.value as typeof settings.readingFontSize })}><option value="small">较小</option><option value="medium">标准</option><option value="large">较大</option></select></SettingRow>
      ...
      <section className="settings-section compact-section">
        ...
        <div className="trusted-summary"><SlidersHorizontal /><span>已信任 {settings.trustedRemoteImageSenders.length} 个远程图片发件人</span></div>
      </section>
