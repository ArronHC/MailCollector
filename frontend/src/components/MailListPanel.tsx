...
function SnoozeMenu({ close, onSelect }: { close: () => void; onSelect: (date: string) => void }) {
  const date = (days: number, hour: number) => { const next = new Date(); next.setDate(next.getDate() + days); next.setHours(hour, 0, 0, 0); return next.toISOString(); };
  return <><div className="menu-heading">延后到</div><MenuButton icon={<Clock3 />} label="今天晚些时候" detail="18:00" onClick={() => { onSelect(date(0, 18)); close(); }} /><MenuButton icon={<Clock3 />} label="明天" detail="08:00" onClick={() => { onSelect(date(1, 8)); close(); }} /><MenuButton icon={<Clock3 />} label="下周一" detail="08:00" onClick={() => { const now = new Date(); const days = ((8 - now.getDay()) % 7) || 7; onSelect(date(days, 8)); close(); }} /></>;
}
