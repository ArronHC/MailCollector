...
fn runtime_settings(app_data: &Path) -> Result<RuntimeSettings, String> {
    ...
    let settings = RuntimeSettings {
        encryption_key: if encryption_key.len() == 64 { encryption_key } else { random_hex(32) },
        api_key: legacy.get("API_KEY").filter(|value| value.len() >= 24).cloned().unwrap_or_else(|| random_hex(32)),
        invite_code: legacy.get("REGISTRATION_INVITE_CODE").filter(|value| value.len() >= 12).cloned().unwrap_or_else(|| "MC-2026-7F4K9Q2P".to_string()),
        allow_private_mail_hosts: legacy.get("ALLOW_PRIVATE_MAIL_HOSTS").cloned().unwrap_or_else(|| "false".to_string()),
        sync_interval_minutes: legacy.get("SYNC_INTERVAL_MINUTES").cloned().unwrap_or_else(|| "5".to_string()),
        initial_sync_limit: legacy.get("INITIAL_SYNC_LIMIT").cloned().unwrap_or_else(|| "100".to_string()),
        max_message_bytes: legacy.get("MAX_MESSAGE_BYTES").cloned().unwrap_or_else(|| "10485760".to_string()),
    };
...
fn migrate_database(app_data: &Path, settings: &RuntimeSettings) -> Result<PathBuf, String> {
    ...
    let source_path = Path::new(LEGACY_ROOT).join("data").join("mail-collector.db");
...
fn start_sidecar(resource_dir: &Path, app_data: &Path, database_path: &Path, settings: &RuntimeSettings) -> Result<SidecarProcess, String> {
    let bundled_runtime = resource_dir.join("resources").join("runtime");
    let runtime_dir = if bundled_runtime.exists() { bundled_runtime } else { resource_dir.join("runtime") };
    ...
    let stdout = OpenOptions::new().create(true).append(true).open(app_data.join("mail-collector.out.log")).map_err(|error| error.to_string())?;
    let stderr = OpenOptions::new().create(true).append(true).open(app_data.join("mail-collector.err.log")).map_err(|error| error.to_string())?;
    ...
    command
        ...
        .env("ALLOW_PRIVATE_MAIL_HOSTS", &settings.allow_private_mail_hosts)
    ...
    Ok(SidecarProcess { child, port, shutdown_token })
}
...
pub fn run() {
    ...
    let app = tauri::Builder::default()
        .setup(move |app| {
            ...
            let resource_dir = standard_windows_path(app.path().resource_dir().map_err(|error| error.to_string())?);
            let sidecar = start_sidecar(&resource_dir, &app_data, &database_path, &settings)?;
            let url = format!("http://127.0.0.1:{}", sidecar.port).parse().map_err(|error| format!("无效的本地地址：{error}"))?;
