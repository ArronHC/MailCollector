...
桌面端的邮件数据始终保存在当前设备。可选择是否启用云配置同步：

- 仅本机：邮箱配置和邮件数据都保存在当前设备。
- 加密配置同步：邮箱连接配置和授权码在客户端使用管理员指定的 256 位同步密钥进行 AES-256-GCM 加密，服务器只保存密文；每台设备自行连接 IMAP 并保存邮件。

同步密钥必须是 64 位十六进制字符，且不会发送给服务器。所有设备必须使用同一密钥；密钥丢失后无法解密云端配置。
...
在桌面端顶部齿轮中打开“数据与同步”，选择“加密配置同步”，填写 HTTPS 地址、服务器 `API_KEY` 和所有设备共用的 64 位管理员同步密钥，然后点击“连接并同步”。API Key 用于服务器认证，同步密钥只用于客户端加解密。

第一台设备也可以直接浏览 `https://mail.example.com`，使用脚本输出的邀请码创建唯一管理员账户。管理员注册完成后会自动关闭再次注册。

多台设备连接同一服务器后：

- 邮箱连接配置只需在任意一台设备添加一次，其他设备会解密并导入。
- 邮件正文、索引、草稿和本地标签不会上传到 Mail Collector 服务器。
- 每台设备独立同步 IMAP，因此会分别下载邮件；已读和星标可通过邮箱服务器自然收敛。
- 删除同步账户时，当前设备删除本地数据，其他设备会停用对应配置并保留其本地邮件。
...
- 桌面版通过 Tauri 应用数据目录保存云配置地址；只有选择“记住”时才会持久化 API Key 和同步密钥。浏览器版使用当前站点的 Web Storage。只应在可信个人设备上记住密钥。
- 首次启用时会把已有本机邮箱连接配置加密上传，但不会上传邮件数据库。
- 云配置服务器启用 `CONFIG_SYNC_ONLY=true` 后不会运行 IMAP、SMTP、IDLE 或邮件 Worker，也不会开放邮件数据接口。
