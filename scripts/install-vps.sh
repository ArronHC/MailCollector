...
MANAGED_OVERRIDE=0
if [[ -f "$OVERRIDE_FILE" ]] && grep -q '^# Managed by Mail Collector installer
if [[ ! -f "$ENV_FILE" ]]; then
  umask 077
  cat > "$ENV_FILE" <<EOF
HOST=0.0.0.0
...
if [[ "$PROXY_MODE" == "auto" ]]; then
  if [[ "$OWN_CADDY_RUNNING" -eq 1 ]]; then
    PROXY_MODE="bundled"
...
if [[ "$PROXY_MODE" == "bundled" ]] && [[ "$OWN_CADDY_RUNNING" -ne 1 ]] && { port_in_use 80 || port_in_use 443; }; then
  echo "Ports 80/443 are already in use. Use --proxy-mode external or leave the default auto mode enabled." >&2
  exit 1
fi
...
if ps -eo comm= 2>/dev/null | grep -qx openresty || docker ps --format '{{.Names}}' 2>/dev/null | grep -qi '1panel-openresty'; then
